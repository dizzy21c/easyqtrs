    ...
    def format_response_data(self, rep_data, prefix=False):
        ...
        for stock_detail in stock_details:
            ...
            stock_dict[stock_code] = {
                ...
                "datetime": datetime.strptime(stock[30], "%Y%m%d%H%M%S").strftime("%Y-%m-%d %H:%M:%S"),
                # "涨跌": float(stock[31]),
                "chg": float(stock[31]),
                # "涨跌(%)": float(stock[32]),
                "chg_pct": float(stock[32]),
                "high": float(stock[33]),
                "low": float(stock[34]),
                # "价格/成交量(手)/成交额": stock[35],
                "price_vol_amount": stock[35],
                ...
                "circulation_market_value": self._safe_float(stock[44]),
                # "总市值": self._safe_float(stock[45]),
                "total_value": self._safe_float(stock[45]),
                "PB": float(stock[46]),
                # "涨停价": float(stock[47]),
                "ceiling_price": float(stock[47]),
                # "跌停价": float(stock[48]),
                "floor_price": float(stock[48]),
                # "量比": self._safe_float(stock[49]),
                #quantity relative ratio
                "QRR": self._safe_float(stock[49]),
                # "委差": self._safe_acquire_float(stock, 50),
                "dapanweicha": self._safe_acquire_float(stock, 50),
                # "均价": self._safe_acquire_float(stock, 51),
                "price_avg": self._safe_acquire_float(stock, 51),
                # "市盈(动)": self._safe_acquire_float(stock, 52),
                "Trailing_PE": self._safe_acquire_float(stock, 52),
                # "市盈(静)": self._safe_acquire_float(stock, 53),
                "S_PE": self._safe_acquire_float(stock, 53),
            }
        return stock_dict
