...
if __name__ == "__main__":
    ...
    dataqout = DataQoute()
    # dataqout.GetShCodes()
    # hk_codes = {"sz300274": {"code": "sz300274", "name": "name"}}
    codelist = QA.QA_fetch_stock_block_adv().code
    st_start="2020-01-01"
    st_end = "2021-12-31"
    mongo_mp = MongoIo()
    mdata=mongo_mp.get_stock_day(codelist, st_start=st_start, st_end=st_end)
    for code in codelist:
        # dataqout.SetCode(code)
        kline_df=mdata.query("code=='%s'" % code)
        if len(kline_df) > 0 :
            kline_df = kline_df.reset_index()
            kline_df['date'] = kline_df['date'].apply(lambda x: x.strftime('%Y-%m-%d'))
            kline_day = json.loads(kline_df.to_json(orient='records'))
            # print(code, kline_day[0])
            # print(kline_day)
            try:
                chan_day=ChanAnalyze(kline_day)
                bthird_buy,price=chan_day.thirdbuy(bstrong=False)
                if bthird_buy:
                    chan_day.draw(code+"_")
            except:
