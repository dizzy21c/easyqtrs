...
def new_df3(df_day, data, now_price):
    last_time = data.select('datetime').row(0)[0][:10]
    copen = data.select('open').row(0)[0]
    close = data.select('close').row(0)[0]
    high = data.select('high').row(0)[0]
    low = data.select('low').row(0)[0]
    volume = data.select('amount').row(0)[0] / 100
    amount = data.select('volume').row(0)[0]
    
#     date = last_time
    code = data.select('code').row(0)[0]
    
    nrow = pl.DataFrame({'open':[copen],'close':[close], 'high':[high], 'low':[low], 'amount':[amount],
                         'date':[last_time], 'code':[code], 'volume':[volume]})
    nrow = nrow.with_column(pl.col("date").str.strptime(pl.Date, fmt="%Y-%m-%d"))
    df2 = df_day.extend(nrow).to_pandas()
    return df2.set_index(['date', 'code'])

def do_get_data_mp(key, codelist, st_start, st_end, calc_type=''):
    mongo_mp = MongoIo4Pl()
    ...
    if calc_type == 'B':
        ...
        databuf_mongo_rn[key] = pl.DataFrame()
...
def get_data(codelist, st_start, st_end, calc_type):
    start_t = datetime.datetime.now()
    ...
    for i in code_dict.keys():
        ...
        pool.apply_async(do_get_data_mp, args=(i, code_dict[i], st_start, st_end, calc_type))
...
def tdx_func_mp(func_names, sort_types, codelist, calc_type='', backTime=''):
    ...
    for func_name in func_nameA:
        ...
        code_dict = codelist2dict(codelist, pool_size)
        # print("get-data", code_dict)
#         executoPool = Pool(1) #Pool(pool_size)
        executorDoCalc = ThreadPoolExecutor(max_workers=pool_size)
        for key in keysObj:
            # tdx_func(databuf_mongo[key])
#             df = databuf_mongo[key].filter(pl.col('code') == '000859')
#             print("calc len =" , len(df))
            task_list.append(executorDoCalc.submit(tdx_func, key, newdatas, func_name, keysObj[key], calc_type))
#             task_list.append(
#             executoPool.apply_async(tdx_func, args=(key, newdatas, func_name, keysObj[key], calc_type))
#             tdx_func(key, newdatas, func_name, keysObj[key], calc_type)
#         executoPool.close()
#         executoPool.join()
    ...
    mongo.upd_backtest(func_names, dataR, backTime, calc_type)
...
def tdx_func(key, newdatas, func_name, code_list = None,calc_type=''):
    """
    准备数据
    """
    start_t = datetime.datetime.now()
...
#         code_list = datam.index.levels[1]
    ...
    for code in code_list:
        data=datam.filter(pl.col('code') == code)
        ...
        try:
            if calc_type == 'B':
                newdata0 = newdatas.filter(pl.col('code') == code)
                if len(newdata0) > 0:
                    newdata = newdata0.tail(1)
                    now_price = newdata.select('now').row(0)[0]
                    last_price = newdata.select('now').row(0)[0]
                else:
            ...
            if calc_type == 'B':
                try:
                    datal = datam_r.filter(pl.col('code') == code)
                    if len(datal) == 0:
                        continue
                    last_price = datal.select('close').row(0)[0]
                except Exception as e:
                    dataln = None
                    print("last-date=0, code=", code, key, e)
                    last_price = 0
                    continue
                try:
                    dataln = datam_rn.filter(pl.col('code') == code)
                except:
                    dataln = None
            # if (code == '003001'):
            #     print(data)
            #     print(newdata)
#             print(data.copy().tail())
#             print(newdata)
#             break
#             print(data.tail(1), newdata['volume'], now_price)
            data = new_df3(data, newdata, now_price)
#             if code == '300039':
#                 print("data", data.tail(2))
            calcR = tdx_base_func(data, func_name, code, newdata, last_price, dataln)
#             print(data.tail(2), calcR)
#             break
            if calcR == {}:
                continue
...
def tdx_base_func(data, func_name, code, newdata, lastPrice, lastNData):
    """
    准备数据
    """
    dateObj = newdata.select('datetime').row(0)[0]
    timeStr = newdata.select('time').row(0)[0]
    insFlg = True
    nowPrice = newdata.select('now').row(0)[0]
    nowOpen = newdata.select('open').row(0)[0]
    oldClose = newdata.select('close').row(0)[0]
    if nowPrice == 0:
        return {}
    if nowOpen == 0:
        nowOpen = nowPrice
...
#     PCT4 = lastPrice / nowOpen - 1
    ...
    if lastNData is None or len(lastNData) == 0:
        ...
        PCTNO = lastNData.select('open').row(0)[0] / nowPrice - 1
        PCTNC = lastNData.select('close').row(0)[0] / nowPrice - 1
        PCTNL = lastNData.select('low').row(0)[0] / nowPrice - 1
        PCTNH = lastNData.select('high').row(0)[0] / nowPrice - 1
...
#         if code == '300039':
#             print("tdx_func_result ", tdx_func_result.tail(3))
        
    # 斜率
    except Exception as e:
        print("calc %s code=%s ERROR:FUNC-CALC-ERROR " % (func_name, code), e)
...
if __name__ == '__main__':
    ...
    # st_start, st_end, func = main_param(sys.argv)
    # print("input", st_start, st_end, func)
    st_start, st_end, func, sort, calc_type, back_time, all_data = main_param(sys.argv)
    if calc_type == 'B':
        m = MongoIo()
        rc = m.get_realtime_count(dateStr = back_time)
        if rc == 0:
...
#         exit(0)
    # st_start = "2019-01-01"
    # func = "test"
    print("input st-start=%s, st-end=%s, func=%s, sort=%s, calc_type=%s, back-time=%s, all_data = %s" % (st_start, st_end, func, sort, calc_type, back_time, all_data))
    # 1, 读取数据（多进程，读入缓冲）
    # 开始日期
    # data_day = get_data(st_start)
    # print(data_day)
    ...
    if calc_type != 'R':
        get_data(codelist, st_start, st_end, calc_type)
    ...
    while True:
        ...
        if calc_type == 'T':
            ...
            time.sleep(10)
        print("*** loop calc begin ***")
        tdx_func_mp(func, sort, codelist, calc_type=calc_type, backTime=back_time)
