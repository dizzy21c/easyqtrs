...
def doForcastMP(idx, baseDate, delta, predictOrg):
    start_t = datetime.datetime.now()
    print("begin-doForcastMP :", start_t, idx)
    # TODO 
    codelist = code_dict[idx]
    # print("codelist", codelist)
    # mongoM = MongoIo()
    dataBuff = databuf_mongo[idx]
    # print("databuff", dataBuff.head())
    dataResult = pd.DataFrame()
    for code in codelist:
        # print("begin-doForcastMP code:", code)
        # dataM=mongoM.get_stock_day(code, st_start = '1990-01-01')
        dataM=dataBuff.query("code=='%s'" % code).copy()
        # if len(dataM)
        # print(type(dataM))
        if len(dataM) == 0:
            # print("dataM size == 0")
        ...
        print("begin-doForcastMP code:", code, len(dataM))
        try:
            ...
            if dictData['code'] == []:
                ...
                if len(dataResult) == 0:
                    ...
                    dataResult = dataResult.append(df)
            print("calc result:", code, predictValue[:20], len(dataResult))
        except Exception as e:
            print("calc doForcast4LoopYear except", e)
    dataResult.to_csv("calc-out-%d.csv" % idx)
    end_t = datetime.datetime.now()
    print(end_t, 'doForcastMP spent:{}, idx={}'.format((end_t - start_t), idx))

    return dataResult

def do_get_data_mp(key, codelist, st_start = '1990-01-01', st_end = '2031-12-31'):
    # print("do-get-data-mp ... ", codelist[:3])
    mongo_mp = MongoIo()
    start_t = datetime.datetime.now()
    ...
    end_t = datetime.datetime.now()
    print(end_t, 'get_data do_get_data_mp spent:{}, size = {}'.format((end_t - start_t), len(databuf_mongo[key])))
...
if __name__ == "__main__":
    ...
    mongo=MongoIo()
    pool_size = cpu_count()
    codelist=list(mongo.get_stock_list().index)
    code_dict = codelist2dict(codelist, pool_size)
    # (pool_size, code_dict) = getCodeList(mongo)
    pool = Pool(cpu_count())
