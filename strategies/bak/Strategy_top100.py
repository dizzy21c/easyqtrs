...
class Strategy(StrategyTemplate):
    ...
    def strategy(self, event):
        ...
        self.log.info('\n\nStrategy top100 event')
        for d in event.data:
            pass
            # time.sleep(1)
            # print("code=%s now=%s" % (d, event.data[d]['now']))
            # close =  event.data[d]['close']
            # now = event.data[d]['now']
            # high = event.data[d]['high']
            # low = event.data[d]['low']
            # if close <= 0:
            #     continue
            # chgValue = now - close
            # pct = ( now - close ) * 100/ close
            # highPct = (high - close) * 100 / close
            # zf = (high - low) * 100 / close
            # lowPct = (high - low) * 100 / close
            # # if pct > 5:
            # self.log.info( "code=%s pct=%6.2f val=%6.2f" % (d, pct, now))
        # self.log.info('\n\nStrategy 1 event')
        # self.log.info('data: code-name: %s' % event.data['000002'])
    ...
    def shutdown(self):
        ...
        self.log.info("save to Strategy")
