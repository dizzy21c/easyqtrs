...
#                 data=datam.query(" code=='%s' and date < '%s' " % (code, calcDate)).copy()
                data=datam.filter((pl.col('code') == code) & (pl.col('date') < datetime.datetime.strptime(calcDate, '%Y-%m-%d'))).to_pandas()
                newdata0 = newdatas.filter((pl.col('code') == code))
                if len(newdata0) > 0:
                    newdata = newdata0.tail(1).to_pandas()
                else:
...
#                     last_price = datal.select(['close']).row(-1)
                ...
                try:
                    ...
                    dataln = datam.filter((pl.col('code') == code) & (pl.col('date') == datetime.datetime.strptime(nextCalcDate, '%Y-%m-%d'))).to_pandas()
                except:
                    dataln = None
            # if (code == '003001'):
            #     print(data)
